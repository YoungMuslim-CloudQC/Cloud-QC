...
import { areaLabel, areaWhere } from "@/lib/sub-regions";
import {
  EVENT_TYPES,
  EVENT_TYPE_LABEL,
  type EventTypeValue,
 ...
 *  the "step two" view before drilling into a single location. */
export async function AreaFeed({
  ...
  const visits = await db.visit.findMany({
    where: {
      deletedAt: null,
      neighbornet: { archivedAt: null, ...areaWhere(area) },
      ...(type ? { eventType: type } : {}),
    },
          ...
          return (
            <div className="feed-row" key={v.id}>
              <div className="feed-row-top">
                ...
                <Link href={`/neighbornets/${v.neighbornet.id}?area=${encodeURIComponent(area)}`} className="visit-nn">
                  {v.neighbornet.name}
                </Link>
